...
def episode_schema_matches(episode: dict[str, Any], expected_schema_version: str | None) -> bool:
    """Episode schema 必须逐个匹配本次 manifest，而不是 gate 内置的历史常量。"""
    return bool(expected_schema_version) and episode.get("memory_schema_version") == expected_schema_version


def tokenize_memory_untruncated(tok: Any, text: str, subtask_max_len: int) -> dict[str, Any]:
    """用当前 MemoryTextCodec 计真长度，并用 tokenize_memory 校验生产 masks。

    tokenize_memory 会 padding，且超长时会直接抛错，因此真长度必须从 codec 的未截断
    body ids 计算。未超长时再走一次真实生产 API，以验证 EOS 与 mask 契约。
    """
    body_ids = tok.memory_codec().encode(text.strip(), validate=True)
    true_len = len(body_ids) + 2  # BOS + body + EOS
    result: dict[str, Any] = {"length": true_len, "encoded": None}
    if true_len <= subtask_max_len:
        tokens, mask, ar_mask, loss_mask = tok.tokenize_memory(text, validate=True)
        result["encoded"] = (tokens, mask, ar_mask, loss_mask)
    return result
...
def scan_episode(path_str: str) -> dict[str, Any]:
    ...
    expected_episode_schema = _OPTS.get("expected_schema_version")
    if not episode_schema_matches(episode, expected_episode_schema):
        note("schema_version", f"{path_str}: episode={episode.get('memory_schema_version')!r} "
             f"manifest={expected_episode_schema!r}")
    ...
    for i, row in enumerate(rows):
        ...
        try:
            prod = tokenize_memory_untruncated(tok, planner, subtask_max_len)
            prod_len = prod["length"]
            encoded = prod["encoded"]
            if encoded is not None:
                prod_tokens, prod_mask, prod_ar_mask, prod_loss_mask = encoded
                prod_n_valid = int(prod_mask.sum())
                prod_eos = np.flatnonzero(prod_tokens[:prod_n_valid] == eos_id)
                if prod_eos.size == 0:
                    note("eos_lost", f"{path_str}#{i}: production len={prod_len}")
                else:
                    prod_last_eos = int(prod_eos[-1])
                    if not bool(prod_loss_mask[prod_last_eos]):
                        note("eos_not_supervised", f"{path_str}#{i}: production")
                    if prod_last_eos != prod_n_valid - 1:
                        note("eos_not_last_valid", f"{path_str}#{i}: production eos@{prod_last_eos}")
                prod_expected_loss = prod_mask.copy()
                prod_expected_loss[0] = False
                if not np.array_equal(prod_loss_mask, prod_expected_loss):
                    note("loss_mask_contract", f"{path_str}#{i}: production")
                if int(prod_tokens[0]) != sp.bos_id():
                    note("bos_missing", f"{path_str}#{i}: production")
                if (int(np.min(prod_ar_mask[:prod_n_valid])) != 1
                        or int(np.max(prod_ar_mask[prod_n_valid:], initial=0)) != 0):
                    note("ar_mask_contract", f"{path_str}#{i}: production")
...
# ---------------------------------------------------------------------------
# sampling manifest：task × transition × rank 的正式训练门禁
# ---------------------------------------------------------------------------
SAMPLING_SPAN_FIELDS = frozenset({
    "anchor_range", "annotation_relative_path", "coverage_status", "data_version",
    "episode", "episode_index", "frame_range", "memory_idx", "planner_update_target",
    "prompt_version", "provenance", "sampling_group", "schema_version", "task",
    "task_id", "task_key", "transition_type", "weight_hint",
})
...
def _normalise_sampling_stats(value: Any) -> dict[tuple[int, str], dict[str, int]]:
    """解析 producer v3 的 canonical per_task_transition_stats 行表。"""
    if not isinstance(value, list):
        raise ValueError("per_task_transition_stats 必须是数组")
    required = {"task_id", "transition_type", "intervals", "frames", "anchors"}
    out: dict[tuple[int, str], dict[str, int]] = {}
    ordered_keys: list[tuple[int, str]] = []
    for item in value:
        if not isinstance(item, dict) or set(item) != required:
            raise ValueError(f"统计行字段必须精确等于 {sorted(required)}: {item!r}")
        task, transition = item["task_id"], item["transition_type"]
        if (isinstance(task, bool) or not isinstance(task, int) or task < 0
                or transition not in ALLOWED_TRANSITIONS):
            raise ValueError(f"统计行 task_id/transition_type 非法: {item!r}")
        counts = {name: item[name] for name in ("intervals", "frames", "anchors")}
        if any(isinstance(count, bool) or not isinstance(count, int) or count < 0
               for count in counts.values()):
            raise ValueError(f"统计行计数必须是非负整数: {item!r}")
        key = (task, transition)
        if key in out:
            raise ValueError(f"重复统计键: {key!r}")
        out[key] = counts
        ordered_keys.append(key)
    if ordered_keys != sorted(ordered_keys):
        raise ValueError("per_task_transition_stats 未按 (task_id, transition_type) 稳定排序")
    return out


def validate_sampling_spans(
    *,
    data_root: Path,
    world_size: int,
    num_workers: int,
    ...
    """全量验证 sampling_spans.jsonl，并模拟 global-worker/rank 分片。

    函数不依赖 tokenizer，便于生产 fail-fast，也便于用小型合成数据做正/负对照。
    所有检查均为 hard；调用者必须把任一失败传播为非零退出。
    """
    checks: list[dict[str, Any]] = []
    evidence: dict[str, list[str]] = {}

    def note(kind: str, message: str) -> None:
        bucket = evidence.setdefault(kind, [])
        if len(bucket) < 12:
            bucket.append(message)

    def check(key: str, passed: bool, detail: str, measured: Any, threshold: Any) -> None:
        checks.append({
            "key": key, "hard": True, "passed": bool(passed), "detail": detail,
            "measured": measured, "threshold": threshold,
            "evidence": evidence.get(key, [])[:12],
        })
    ...
    try:
        ...
        if not isinstance(raw_manifest, dict):
            raise ValueError("manifest 顶层不是对象")
        manifest = raw_manifest
    except Exception as exc:  # noqa: BLE001
        manifest_error = f"{type(exc).__name__}: {exc}"
    check("SAMPLING_MANIFEST_READABLE", manifest is not None,
          "formal 模式必须存在且可解析 manifest.json", manifest_error or str(manifest_path), "可读 JSON object")
    if manifest is None:
        return {"verdict": "FAIL", "checks": checks, "hard_failures": [c["key"] for c in checks if not c["passed"]],
                "evidence": evidence, "rank_coverage": {}, "records": 0}

    required_manifest = {
        "sampling_spans", "per_task_transition_stats", "data_version", "prompt_version", "schema_version",
    }
    missing_manifest = sorted(required_manifest - set(manifest))
    legacy_manifest = sorted({
        "sampling_spans_path", "sha256", "records", "sampling_stats_by_task_transition",
    }.intersection(manifest))
    sampling_meta = manifest.get("sampling_spans")
    required_sampling_meta = {"relative_path", "sha256", "count", "interval_convention", "schema_version"}
    sampling_meta_shape_ok = isinstance(sampling_meta, dict) and set(sampling_meta) == required_sampling_meta
    sampling_schema_ok = (sampling_meta_shape_ok
                          and isinstance(sampling_meta["schema_version"], str)
                          and bool(sampling_meta["schema_version"]))
    interval_convention_ok = (sampling_meta_shape_ok
                              and sampling_meta["interval_convention"] == EXPECTED_INTERVAL_CONVENTION)
    check("SAMPLING_MANIFEST_CONTRACT",
          not missing_manifest and not legacy_manifest and sampling_meta_shape_ok
          and sampling_schema_ok and interval_convention_ok,
          "manifest 必须严格使用 producer v3 的 nested sampling_spans 与 per_task_transition_stats 契约",
          {"missing": missing_manifest, "legacy_top_level_fields": legacy_manifest,
           "sampling_spans_keys": sorted(sampling_meta) if isinstance(sampling_meta, dict) else None,
           "sampling_schema_version": (sampling_meta or {}).get("schema_version")
           if isinstance(sampling_meta, dict) else None,
           "interval_convention": (sampling_meta or {}).get("interval_convention")
           if isinstance(sampling_meta, dict) else None},
          {"required_top_level": sorted(required_manifest),
           "sampling_spans_fields": sorted(required_sampling_meta),
           "interval_convention": EXPECTED_INTERVAL_CONVENTION})

    rel = sampling_meta.get("relative_path") if sampling_meta_shape_ok else None
    ...
    if isinstance(rel, str) and rel:
        ...
        try:
            ...
            path_error = "sampling_spans.relative_path 逃逸 data_root"
    else:
        path_error = "sampling_spans.relative_path 不是非空字符串"
    exists = spans_path is not None and spans_path.is_file()
    check("SAMPLING_SPANS_EXISTS", exists,
          "formal 模式必须存在 manifest 指向的 sampling_spans.jsonl",
          {"path": str(spans_path) if spans_path else rel, "error": path_error}, "regular file under data_root")
    if not exists:
        return {"verdict": "FAIL", "checks": checks, "hard_failures": [c["key"] for c in checks if not c["passed"]],
                "evidence": evidence, "rank_coverage": {}, "records": 0}

    expected_sha = sampling_meta.get("sha256") if sampling_meta_shape_ok else None
    hash_error = None
    try:
        actual_sha = _sha256_file(spans_path)
    except Exception as exc:  # noqa: BLE001
        actual_sha = None
        hash_error = f"{type(exc).__name__}: {exc}"
    check("SAMPLING_SPANS_SHA256", actual_sha is not None and isinstance(expected_sha, str)
          and actual_sha == expected_sha.lower(),
          "sampling_spans 文件 SHA-256 必须与 manifest 逐字节一致",
          {"actual": actual_sha, "manifest": expected_sha, "error": hash_error}, "equal")
    ...
    try:
        with spans_path.open(encoding="utf-8") as fh:
            for line_no, line in enumerate(fh, 1):
                if not line.strip():
                    parse_errors += 1
                    note("SAMPLING_SPANS_JSONL", f"line {line_no}: 空行")
                    continue
                try:
                    row = json.loads(line)
                    if not isinstance(row, dict):
                        raise ValueError("行不是 object")
                    rows.append(row)
                except Exception as exc:  # noqa: BLE001
                    parse_errors += 1
                    note("SAMPLING_SPANS_JSONL", f"line {line_no}: {type(exc).__name__}: {exc}")
    except Exception as exc:  # noqa: BLE001
        parse_errors += 1
        note("SAMPLING_SPANS_JSONL", f"文件读取失败: {type(exc).__name__}: {exc}")
    check("SAMPLING_SPANS_JSONL", parse_errors == 0,
          "JSONL 必须每个物理行恰好包含一个 JSON object，禁止空行", parse_errors, 0)
    expected_records = sampling_meta.get("count") if sampling_meta_shape_ok else None
    records_ok = (not isinstance(expected_records, bool) and isinstance(expected_records, int)
                  and expected_records == len(rows))
    check("SAMPLING_SPANS_RECORDS", records_ok,
          "可解析 record 数必须与 manifest 一致", {"actual": len(rows), "manifest": expected_records}, "equal")
    ...
    path_to_episode: dict[str, tuple[int, int]] = {}
    episode_to_path: dict[tuple[int, int], str] = {}
    seen_keys: set[tuple[Any, ...]] = set()
    seen_anchors: set[tuple[int, int, int]] = set()
    sort_keys: list[tuple[Any, ...]] = []
    trainable: list[tuple[int, str, bool]] = []

    for index, row in enumerate(rows):
        line_no = index + 1
        missing = sorted(SAMPLING_SPAN_FIELDS - set(row))
        extra = sorted(set(row) - SAMPLING_SPAN_FIELDS)
        if missing or extra:
            schema_bad += 1
            note("SAMPLING_ROW_SCHEMA_VERSION", f"line {line_no}: missing={missing} extra={extra}")

        top_versions_match = all(isinstance(row.get(k), str) and row.get(k)
                                 and row.get(k) == manifest.get(k)
                                 for k in ("data_version", "prompt_version"))
        row_schema = row.get("schema_version")
        sampling_schema_match = (isinstance(row_schema, str) and bool(row_schema)
                                 and sampling_meta_shape_ok
                                 and row_schema == sampling_meta.get("schema_version"))
        if not top_versions_match or not sampling_schema_match:
            version_bad += 1
            note("SAMPLING_ROW_SCHEMA_VERSION",
                 f"line {line_no}: data/prompt 顶层版本或 sampling_spans.schema_version 不一致")
        ...
        memory_idx = row.get("memory_idx")
        transition = row.get("transition_type")
        coverage = row.get("coverage_status")
        planner_target = row.get("planner_update_target")
        sampling_group = row.get("sampling_group")
        weight_hint = row.get("weight_hint")
        provenance = row.get("provenance")
        planner_fields = {"current_primitive", "current_skill", "next_primitive", "next_skill"}
        planner_ok = isinstance(planner_target, dict) and all(
            isinstance(planner_target.get(name), str) and planner_target.get(name).strip()
            for name in planner_fields
        )
        if (not isinstance(coverage, str) or not coverage
                or not planner_ok
                or not isinstance(sampling_group, str) or not sampling_group
                or isinstance(weight_hint, bool) or not isinstance(weight_hint, (int, float))
                or not np.isfinite(weight_hint) or weight_hint < 0
                or not isinstance(provenance, dict)):
            schema_bad += 1
            note("SAMPLING_ROW_SCHEMA_VERSION", f"line {line_no}: coverage/target/group/weight/provenance 非法")
        if (isinstance(task_id, bool) or not isinstance(task_id, int) or task_id < 0
                or not isinstance(task, str) or not task
                or not isinstance(task_key, str) or not task_key
                or not isinstance(episode, str) or not episode
                or isinstance(episode_index, bool) or not isinstance(episode_index, int) or episode_index < 0
                or isinstance(memory_idx, bool) or not isinstance(memory_idx, int) or memory_idx < 0
                or not isinstance(transition, str) or transition not in ALLOWED_TRANSITIONS):
            schema_bad += 1
            note("SAMPLING_ROW_SCHEMA_VERSION", f"line {line_no}: task/episode/memory/transition 类型非法")
            continue
        stat = stats.setdefault((task_id, transition), {"intervals": 0, "frames": 0, "anchors": 0})
        stat["intervals"] += 1

        def parse_range(name: str, *, allow_empty: bool) -> tuple[int, int] | None:
            nonlocal range_bad
            value = row.get(name)
            good_shape = isinstance(value, list) and len(value) == 2
            if not good_shape or any(isinstance(x, bool) or not isinstance(x, int) for x in (value or [])):
                range_bad += 1
                note("SAMPLING_HALF_OPEN_RANGES", f"line {line_no}: {name}={value!r}")
                return None
            start, end = value
            if start < 0 or end < start or (not allow_empty and start == end):
                range_bad += 1
                note("SAMPLING_HALF_OPEN_RANGES", f"line {line_no}: {name}=[{start},{end})")
                return None
            return start, end

        frame = parse_range("frame_range", allow_empty=False)
        dead = isinstance(coverage, str) and "dead" in coverage.lower()
        anchor_value = row.get("anchor_range")
        anchor = None
        if anchor_value is not None:
            anchor = parse_range("anchor_range", allow_empty=dead)
        elif not dead:
            range_bad += 1
            note("SAMPLING_HALF_OPEN_RANGES", f"line {line_no}: live row anchor_range=None")

        anchor_count = 0 if anchor is None else max(0, anchor[1] - anchor[0])
        if frame is not None:
            stat["frames"] += frame[1] - frame[0]
        stat["anchors"] += anchor_count
        if frame is not None and anchor is not None and not (frame[0] <= anchor[0] <= anchor[1] <= frame[1]):
            range_bad += 1
            note("SAMPLING_HALF_OPEN_RANGES", f"line {line_no}: anchor {anchor} 不在 frame {frame} 内")
        if dead and anchor_count:
            dead_trainable += anchor_count
            note("SAMPLING_DEAD_NOT_TRAINABLE", f"line {line_no}: dead 仍有 {anchor_count} anchors")
        if not dead and anchor_count == 0:
            range_bad += 1
            note("SAMPLING_HALF_OPEN_RANGES", f"line {line_no}: 非 dead 但无 anchor")
        if not dead and anchor is not None:
            update_markers = f"{sampling_group} {transition}".lower()
            is_update = "bridge" in update_markers or "update" in update_markers
            for anchor_index in range(anchor[0], anchor[1]):
                anchor_key = (task_id, episode_index, anchor_index)
                if anchor_key in seen_anchors:
                    duplicate_keys += 1
                    note("SAMPLING_STABLE_SORT_UNIQUE", f"line {line_no}: duplicate anchor={anchor_key!r}")
                seen_anchors.add(anchor_key)
                trainable.append((task_id, transition, is_update))

        ann = row.get("annotation_relative_path")
        if not isinstance(ann, str) or not ann or Path(ann).is_absolute() or ".." in Path(ann).parts:
            path_bad += 1
            note("SAMPLING_ANNOTATION_PATH_KEY", f"line {line_no}: 非法相对路径 {ann!r}")
        else:
            identity = (task_id, episode_index)
            if ann in path_to_episode and path_to_episode[ann] != identity:
                path_bad += 1
                note("SAMPLING_ANNOTATION_PATH_KEY", f"line {line_no}: {ann!r} 映射多个 episode")
            if identity in episode_to_path and episode_to_path[identity] != ann:
                path_bad += 1
                note("SAMPLING_ANNOTATION_PATH_KEY", f"line {line_no}: {identity!r} 映射多个 path")
            path_to_episode[ann] = identity
            episode_to_path[identity] = ann

        key = (task_id, episode_index, memory_idx,
               json.dumps(row.get("frame_range"), sort_keys=True, separators=(",", ":")),
               json.dumps(row.get("anchor_range"), sort_keys=True, separators=(",", ":")),
               str(row.get("sampling_group")), transition)
        if key in seen_keys:
            duplicate_keys += 1
            note("SAMPLING_STABLE_SORT_UNIQUE", f"line {line_no}: duplicate={key!r}")
        seen_keys.add(key)
        sort_keys.append(key)

    check("SAMPLING_ROW_SCHEMA_VERSION", schema_bad == 0 and version_bad == 0,
          "每行字段集合必须精确匹配契约；data/prompt 版本匹配顶层，schema 匹配 sampling_spans.schema_version",
          {"schema_errors": schema_bad, "version_errors": version_bad}, 0)
    check("SAMPLING_HALF_OPEN_RANGES", range_bad == 0,
          "frame/anchor 使用非负半开区间，非 dead anchor 非空且完全落在 frame_range 内", range_bad, 0)
    check("SAMPLING_DEAD_NOT_TRAINABLE", dead_trainable == 0,
          "coverage_status=dead 的 span 不得产生任何可训练 anchor", dead_trainable, 0)
    check("SAMPLING_ANNOTATION_PATH_KEY", path_bad == 0,
          "annotation_relative_path 与 (task_id, episode_index) 必须一一对应且不得逃逸 data_root",
          path_bad, 0)
    sorted_ok = sort_keys == sorted(sort_keys)
    if not sorted_ok:
        note("SAMPLING_STABLE_SORT_UNIQUE", "JSONL 行未按 canonical key 稳定升序")
    check("SAMPLING_STABLE_SORT_UNIQUE", sorted_ok and duplicate_keys == 0,
          "sampling rows 必须按 canonical key 稳定排序且无重复", {"sorted": sorted_ok, "duplicates": duplicate_keys},
          {"sorted": True, "duplicates": 0})

    try:
        manifest_stats = _normalise_sampling_stats(manifest.get("per_task_transition_stats"))
        stats_error = None
    except Exception as exc:  # noqa: BLE001
        manifest_stats = {}
        stats_error = f"{type(exc).__name__}: {exc}"
    stats_equal = stats_error is None and dict(stats) == manifest_stats and bool(stats)
    check("SAMPLING_TASK_TRANSITION_STATS", stats_equal,
          "逐 task×transition 的 intervals/frames/anchors 必须非空并与 manifest 完全一致",
          {"actual": {f"{k[0]}::{k[1]}": v for k, v in sorted(stats.items())},
           "manifest": {f"{k[0]}::{k[1]}": v for k, v in sorted(manifest_stats.items())},
           "error": stats_error}, "exact and globally non-zero")

    topology_ok = (not isinstance(world_size, bool) and isinstance(world_size, int) and world_size > 0
                   and not isinstance(num_workers, bool) and isinstance(num_workers, int) and num_workers > 0)
    rank_coverage: dict[str, list[int]] = {
        f"{task}::{transition}": [0] * max(0, world_size)
        for task, transition in sorted(stats)
    }
    rank_totals = [0] * max(0, world_size)
    gnw = world_size * num_workers if topology_ok else 0
    group_anchor_totals: Counter[tuple[int, str]] = Counter()
    group_update: dict[tuple[int, str], bool] = {}
    if topology_ok:
        for global_index, (task, transition, update) in enumerate(trainable):
            rank = (global_index % gnw) // num_workers
            key = (task, transition)
            label = f"{task}::{transition}"
            rank_coverage.setdefault(label, [0] * world_size)[rank] += 1
            rank_totals[rank] += 1
            group_anchor_totals[key] += 1
            group_update[key] = group_update.get(key, False) or update
    check("SAMPLING_SHARD_EVERY_RANK_NONEMPTY", topology_ok and bool(trainable) and min(rank_totals, default=0) > 0,
          "按 global_index % (world_size*num_workers) 确定性分片后每个 rank 必须有可训练 anchor",
          {"trainable_anchors": len(trainable), "rank_totals": rank_totals,
           "world_size": world_size, "num_workers": num_workers}, {"min_per_rank": 1})

    coverage_failures: list[dict[str, Any]] = []
    for key, total in sorted(group_anchor_totals.items()):
        label = f"{key[0]}::{key[1]}"
        counts = rank_coverage[label]
        nonzero = sum(x > 0 for x in counts)
        bridge_or_update = "bridge" in key[1].lower() or group_update.get(key, False)
        if total < min_samples or (bridge_or_update and nonzero < min(min_ranks, world_size)):
            coverage_failures.append({"group": label, "samples": total, "nonzero_ranks": nonzero,
                                      "bridge_or_update": bridge_or_update})
    # 有 record 却没有 trainable anchor 的 task×transition 也必须显式失败，而不是从矩阵消失。
    for key in sorted(stats):
        if key not in group_anchor_totals:
            coverage_failures.append({"group": f"{key[0]}::{key[1]}", "samples": 0,
                                      "nonzero_ranks": 0, "bridge_or_update": "bridge" in key[1].lower()})
    nonzero_rank_coverage = {
        label: [rank for rank, count in enumerate(counts) if count > 0]
        for label, counts in sorted(rank_coverage.items())
    }
    check("SAMPLING_TASK_TRANSITION_RANK_COVERAGE", not coverage_failures,
          "所有 task×transition 全局样本数达到门槛；bridge/update 至少覆盖配置的 rank 数（不要求稀有组覆盖所有 rank）",
          {"failures": coverage_failures, "by_group_rank_counts": rank_coverage,
           "by_group_nonzero_ranks": nonzero_rank_coverage},
          {"min_samples": min_samples,
           "bridge_update_min_ranks": min(min_ranks, world_size) if world_size > 0 else min_ranks})

    hard_failures = [c["key"] for c in checks if not c["passed"]]
    return {
        "verdict": "PASS" if not hard_failures else "FAIL", "checks": checks,
        "hard_failures": hard_failures, "evidence": evidence, "records": len(rows),
        "trainable_anchors": len(trainable), "sha256": actual_sha,
        "rank_totals": rank_totals, "rank_coverage": rank_coverage,
        "nonzero_rank_coverage": nonzero_rank_coverage,
    }
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--sampling-min-ranks", type=int, default=2,
                   help="bridge 或 planner-update task×transition 至少落到多少个 rank")
    p.add_argument("--sampling-min-samples", type=int, default=1,
                   help="每个 task×transition 的全局可训练 anchor 数下限")
    ...
    for name in ("world_size", "num_workers", "sampling_min_ranks", "sampling_min_samples"):
        value = getattr(args, name)
        if value is not None and value < 1:
            p.error(f"--{name.replace('_', '-')} 必须 >= 1")
...
def main(argv: list[str] | None = None) -> int:
    ...
    sampling_report = validate_sampling_spans(
        data_root=root,
        world_size=args.world_size,
        num_workers=args.num_workers,
        min_ranks=args.sampling_min_ranks,
        min_samples=args.sampling_min_samples,
    )
    sampling_formal = args.mode == "formal"
    if not sampling_formal:
        sampling_failures = list(sampling_report["hard_failures"])
        unavailable_keys = {
            "SAMPLING_MANIFEST_READABLE", "SAMPLING_MANIFEST_CONTRACT", "SAMPLING_SPANS_EXISTS",
        }
        sampling_report["formal_verdict"] = sampling_report["verdict"]
        sampling_report["verdict"] = (
            "NOT-MEASURED" if unavailable_keys.intersection(sampling_failures)
            else ("WARN" if sampling_failures else "PASS")
        )
        sampling_report["warnings"] = sampling_failures
        sampling_report["hard_failures"] = []
        for item in sampling_report["checks"]:
            item["hard"] = False
            if not item["passed"] and item["key"] in unavailable_keys:
                item["detail"] = "NOT-MEASURED：sampling manifest/产物不可用；" + item["detail"]
    for item in sampling_report["checks"]:
        ledger.add(
            item["key"], hard=sampling_formal, passed=item["passed"], detail=item["detail"],
            measured=item["measured"], threshold=item["threshold"], evidence=item.get("evidence"),
        )
    if sampling_formal and sampling_report["verdict"] != "PASS":
        report = {
            "verdict": "FAIL", "generated_at": time.strftime("%Y-%m-%dT%H:%M:%S"),
            "mode": args.mode, "data_root": str(root), "sampling": sampling_report,
            "checks": [c.as_dict() for c in ledger.checks],
            "hard_failures": [c.key for c in ledger.hard_failures],
        }
        report_path = args.out or (root / "moma_pretrain_gate_report.json")
        report_path.parent.mkdir(parents=True, exist_ok=True)
        report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        print(json.dumps(report, ensure_ascii=False, indent=2))
        print(f"GATE_VERDICT: FAIL report={report_path}")
        return 1
    ...
    if n_files == 0:
        report = {
            "verdict": "CANNOT-ASSESS", "mode": args.mode, "data_root": str(root),
            "sampling": sampling_report, "reason": "0 episode files enumerated",
            "checks": [c.as_dict() for c in ledger.checks],
        }
        report_path = args.out or (root / "moma_pretrain_gate_report.json")
        report_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    opts = {
        ...
        "seed": args.seed,
        "expected_schema_version": fingerprint.get("manifest_schema_version"),
    }
    ...
    if readable != n_files:
        ...
        report = {"verdict": "CANNOT-ASSESS", "readable": readable, "files": n_files,
                  "mode": args.mode, "sampling": sampling_report,
                  "checks": [c.as_dict() for c in ledger.checks],
                  "hard_failures": [c.key for c in ledger.hard_failures]}
        report_path = args.out or (root / "moma_pretrain_gate_report.json")
        report_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    report = {
        ...
        "thresholds": {
            ...
            "world_size": args.world_size,
            "num_workers": args.num_workers,
            "sampling_min_ranks": args.sampling_min_ranks,
            "sampling_min_samples": args.sampling_min_samples,
    ...
    report_path = args.out or ((root / "moma_pretrain_gate_report.json") if args.mode == "formal" else None)
    if report_path:
        report_path.parent.mkdir(parents=True, exist_ok=True)
