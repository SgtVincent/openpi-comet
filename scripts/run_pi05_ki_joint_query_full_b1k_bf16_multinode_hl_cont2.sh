...
# CONTINUATION PHASE 2. Phase 1 (offsets 0,4,8) ran its full 104,912-step cosine
# schedule down to a final LR of 2.29e-15, i.e. it stopped by schedule rather
# than by convergence while validation loss was still falling. This launcher
# restarts from the phase-1 step-104912 checkpoint with a fresh cosine (warmup
# 1000, peak LR 1e-5, decay to 0) over a second budget of 104,903 steps -- the
# exact floor-safe capacity of the phase-2 offsets (34979+34968+34956), so every
# pass is exact-fit and no anchor is re-exposed.
# Fixed contract: B8 x W32, GA1, 104,903 steps across streaming stride-12
# episode-local offsets (1,5,9) -- disjoint from the phase-1 offsets (0,4,8), so
# every consumed anchor is fresh -- complete H32 targets only, fixed warmup 1000
# and peak LR 1e-5, online Byted-W&B, and baseline stride-1 validation.
# Normalization statistics are inherited unchanged from phase 1 (the warm-start
# checkpoint's own assets); this is required so the learned action scaling stays
# valid. Per-offset exposure is not re-measured; this mode does not claim exact
# unique coverage and does not support formal resume.
...
CONFIG_NAME="${CONFIG_NAME:-pi05_ki_joint_query_b1k-full_task-ki_on_bf16_cont2}"
if [[ "${CONFIG_NAME}" != "pi05_ki_joint_query_b1k-full_task-ki_on_bf16_cont2" ]]; then
  echo "ERROR: this launcher is restricted to pi05_ki_joint_query_b1k-full_task-ki_on_bf16_cont2." >&2
  exit 2
fi
...
# Continuation warm start: the phase-1 final checkpoint (step 104912). This
# supplies BOTH the initial weights (--pytorch-weight-path) and the normalization
# assets resolved below, so phase 2 inherits phase-1 action scaling unchanged.
BASE_PI05_CKPT="${BASE_PI05_CKPT:-${REPO_ROOT}/outputs/pi05_ki_joint_query_b1k-full_task-ki_on_bf16/checkpoints/pi05_ki_full_b1k_b8_s12_ws360k_20260807/104912}"
...
# Training checkpoints do not carry a HuggingFace-style config.json, but the
# loader path below hard-requires one. Stage it once from the pi05 base
# checkpoint. Idempotent: a pre-existing non-empty config.json is left alone.
CONFIG_JSON_SOURCE="${CONFIG_JSON_SOURCE:-${REPO_ROOT}/checkpoints/pi05_base_pytorch/config.json}"
if [[ ! -s "${BASE_PI05_CKPT}/config.json" ]]; then
  if [[ ! -s "${CONFIG_JSON_SOURCE}" ]]; then
    echo "ERROR: cannot stage config.json; source missing: ${CONFIG_JSON_SOURCE}" >&2
    exit 1
  fi
  _ckpt_config_tmp="${BASE_PI05_CKPT}/config.json.tmp.$$"
  cp -f "${CONFIG_JSON_SOURCE}" "${_ckpt_config_tmp}"
  mv -f "${_ckpt_config_tmp}" "${BASE_PI05_CKPT}/config.json"
  echo "STAGED config.json into warm-start checkpoint from ${CONFIG_JSON_SOURCE}"
fi
if ! cmp -s "${CONFIG_JSON_SOURCE}" "${BASE_PI05_CKPT}/config.json"; then
  echo "ERROR: warm-start config.json differs from ${CONFIG_JSON_SOURCE}; refusing to continue." >&2
  exit 1
fi
...
FRAME_ANCHOR_OFFSETS="${FRAME_ANCHOR_OFFSETS:-1,5,9}"
DROP_INCOMPLETE_ACTION_HORIZON="${DROP_INCOMPLETE_ACTION_HORIZON:-1}"
if [[ "${FRAME_ANCHOR_STRIDE}" != "12" || "${FRAME_ANCHOR_OFFSETS}" != "1,5,9" ]]; then
  echo "ERROR: continuation lean B1K requires FRAME_ANCHOR_STRIDE=12 and FRAME_ANCHOR_OFFSETS=1,5,9." >&2
  echo "       Phase 1 already consumed offsets 0,4,8; reusing them would retrain on seen anchors." >&2
  exit 2
fi
...
# Pass 0 reads this value directly: train_accelerate.py only calls
# _set_formal_b1k_pass_offset() for pass_index > 0, so the first pass's offset is
# whatever is exported here. It MUST be the first offset of the plan (1), not 0,
# or pass 0 would silently re-consume phase-1 anchors. train_accelerate.py's
# _validate_formal_b1k_contract() re-checks this against the registered plan.
export OPENPI_B1K_ANCHOR_OFFSET="1"
...
NUM_TRAIN_STEPS="${NUM_TRAIN_STEPS:-104903}"
...
if [[ -n "${NUM_TRAIN_EPOCHS:-}" ]]; then
  echo "ERROR: NUM_TRAIN_EPOCHS is unsupported; continuation lean B1K uses the fixed 104,903-step budget." >&2
  exit 2
fi
if [[ "${NUM_TRAIN_STEPS}" != "104903" || "${BATCH_SIZE_PER_GPU}" != "8" || "${GRADIENT_ACCUMULATION_STEPS}" != "1" ]]; then
  echo "ERROR: continuation lean B1K requires NUM_TRAIN_STEPS=104903, BATCH_SIZE_PER_GPU=8, and GRADIENT_ACCUMULATION_STEPS=1." >&2
  echo "       104903 is the exact floor-safe capacity of offsets 1,5,9 (34979+34968+34956);" >&2
  echo "       phase 1's 104912 would over-subscribe every pass and re-expose anchors." >&2
  exit 2
fi
...
# Contract-only validation mode. Everything above this point is validation plus
# node-local cache/tokenizer bootstrap: pinned anchor/budget/LR/precision
# contract, topology arithmetic, required-file existence (including the staged
# warm-start config.json and inherited norm stats), dataset root, and pinned
# dependency versions. Node-local cache dirs under LOCAL_CACHE_ROOT may have been
# created, but no persistent output tree exists yet and no distributed process
# has been started. Exiting here lets an operator prove the contract
# accepts/rejects a given env without burning GPUs.
if [[ "${CONTRACT_CHECK_ONLY:-0}" == "1" ]]; then
  echo "CONTRACT_CHECK: CONFIG_NAME=${CONFIG_NAME}"
  echo "CONTRACT_CHECK: FRAME_ANCHOR_STRIDE=${FRAME_ANCHOR_STRIDE} FRAME_ANCHOR_OFFSETS=${FRAME_ANCHOR_OFFSETS}"
  echo "CONTRACT_CHECK: OPENPI_B1K_ANCHOR_OFFSET=${OPENPI_B1K_ANCHOR_OFFSET} (pass-0 offset)"
  echo "CONTRACT_CHECK: DROP_INCOMPLETE_ACTION_HORIZON=${DROP_INCOMPLETE_ACTION_HORIZON}"
  echo "CONTRACT_CHECK: NUM_TRAIN_STEPS=${NUM_TRAIN_STEPS} WARMUP_STEPS=${WARMUP_STEPS} PEAK_LR=${PEAK_LR}"
  echo "CONTRACT_CHECK: BATCH_SIZE_PER_GPU=${BATCH_SIZE_PER_GPU} GRADIENT_ACCUMULATION_STEPS=${GRADIENT_ACCUMULATION_STEPS}"
  echo "CONTRACT_CHECK: PRECISION=${PYTORCH_TRAINING_PRECISION} CHECKPOINT_POLICY=${CHECKPOINT_POLICY} SAVE_INTERVAL=${SAVE_INTERVAL}"
  echo "CONTRACT_CHECK: BASE_PI05_CKPT=${BASE_PI05_CKPT}"
  echo "CONTRACT_CHECK: NORM_STATS_PATH=${NORM_STATS_PATH}"
  echo "CONTRACT_CHECK: TOPOLOGY=${NUM_NODES}x${GPUS_PER_NODE} world=${TOTAL_GPUS}"
  echo "CONTRACT_CHECK_OK; no persistent output tree created, no training launched"
  exit 0
fi
...
echo "BUDGET=${NUM_TRAIN_STEPS} optimizer steps; pass boundaries=34979/69947/104903 (exact-fit)"
echo "PASSES=offset1 source8954823 steps34979; offset5 source8951844 steps34968; offset9 source8948791 steps34956 (each pass == measured floor-safe capacity)"
echo "EXPOSURE=phase-2 anchors are disjoint from phase-1 offsets 0,4,8 under stride 12, so all consumed anchors are fresh; budget equals measured capacity (104903) so no anchor is re-exposed and no pass runs dry; exact unique coverage is not claimed"
echo "WARM_START=phase-1 step-104912 checkpoint; norm stats inherited unchanged (no re-fit)"
