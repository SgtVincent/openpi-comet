...
#
# Two-package FAST controlled experiment. Both package variants use the same
# FAST model/objective and differ only in the coupled warm-start package selected
# by OPENPI_H20_ARM:
#
#   ARM=A  "comet_base package"  config pi05_ki_joint_fast_b1k-full_task-ki_on_h20_bf16
#          weights checkpoints/openpi_comet/pi05-b1kpt50-cs32 (fp32)
#          assets  <that ckpt>/assets      norm_stats sha256 d66ed168…  6368 B
#   ARM=B  "pi05_base package"   config pi05_ki_joint_fast_b1k-full_task-ki_on_h20_pi05base_bf16
#          weights checkpoints/pi05_base_pytorch (bf16)
#          assets  <that ckpt>/assets      norm_stats sha256 4dde119e…  6361 B
#
# Each package variant pairs weights with the normalization they were fit under.
# That is deliberate, and it means the packages differ in TWO coupled variables
# (weights AND action normalization): this comparison ranks base packages; it is
# not the separate FAST-vs-Query model-arm comparison and does not isolate weights.
#
# Contract (identical for both package variants):
...
eval "$("${OPENPI_PREFLIGHT_PYTHON}" -m openpi.training.launcher_profile \
  "${EXPECTED_CONFIG}" --expected-model "${EXPECTED_MODEL}" --world-size 32 --format shell)" \
  || die "failed to resolve exact registered TrainConfig ${EXPECTED_CONFIG}"
[[ "${CFG_NAME}" == "${EXPECTED_CONFIG}" ]] \
  || die "resolved config identity mismatch: ${CFG_NAME} != ${EXPECTED_CONFIG}"
...
# ---- Resolved recipe --------------------------------------------------------
# Recipe values come only from the exact registered TrainConfig resolved above.
...
"${OPENPI_PREFLIGHT_PYTHON}" - "${EXPECTED_CONFIG}" "${EXPECTED_MODEL}" "${MODE}" \
  "${BASE_PI05_CKPT}" "${B1K_DATASET_ROOT}" "${TOTAL_GPUS}" <<'PY'
import sys
from openpi.training.train_config import get_config
name, model_name, mode, base_ckpt, data_root, launcher_world = sys.argv[1:]
...
info "${CFG_PYTORCH_TRAINING_PRECISION} ZeRO-2 no-optimizer-offload B${CFG_BATCH_SIZE_PER_GPU}/GPU GA${CFG_GRADIENT_ACCUMULATION_STEPS} global_batch=${GLOBAL_BATCH_SIZE} gradient_checkpointing=${CFG_GRADIENT_CHECKPOINTING}"
...
info "recipe steps=${CFG_NUM_TRAIN_STEPS} epochs=${CFG_NUM_TRAIN_EPOCHS:-None} stride=${CFG_STREAMING_ANCHOR_STRIDE} warmup=${CFG_WARMUP_STEPS} peak_lr=${CFG_PEAK_LR} decay=${CFG_DECAY_STEPS}->${CFG_DECAY_LR}"
info "cadence save=${CFG_EFFECTIVE_SAVE_INTERVAL} val=${CFG_EFFECTIVE_VAL_LOG_INTERVAL}x${CFG_VAL_NUM_BATCHES} workers=${CFG_NUM_WORKERS} cap=${CFG_ACTION_TOKEN_MAX_LEN}"
info "WARNING: world32 B${CFG_BATCH_SIZE_PER_GPU}/GA${CFG_GRADIENT_ACCUMULATION_STEPS} no-offload peak on H20 is UNMEASURED at this batch; OOM would be a training crash (keepalive-covered), not a lost allocation"
info "wandb_project=${CFG_PROJECT_NAME} output_root=${PERSISTENT_OUTPUT_ROOT}"
info "============================================================"
