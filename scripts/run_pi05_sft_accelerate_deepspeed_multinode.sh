...
# ============================================================
# 4-Node Accelerate + DeepSpeed ZeRO-2 Training Script
...
# Example (4 nodes × 8 GPUs):     3 × 32 × 3 = 288
# ============================================================
...
GRADIENT_ACCUMULATION_STEPS="${GRADIENT_ACCUMULATION_STEPS:-${REF_GRADIENT_ACCUMULATION_STEPS}}"
PYTORCH_TRAINING_PRECISION="${PYTORCH_TRAINING_PRECISION:-bfloat16}"

# Compute a per-GPU micro-batch that preserves the same effective global batch size.
# If you want to override it explicitly, export BATCH_SIZE_PER_GPU before running.
if [[ -z "${BATCH_SIZE_PER_GPU:-}" ]]; then
  ...
  if (( TARGET_GLOBAL_BATCH_SIZE % denom != 0 )); then
    echo "TARGET_GLOBAL_BATCH_SIZE (${TARGET_GLOBAL_BATCH_SIZE}) is not divisible by TOTAL_GPUS*GRAD_ACCUM (${denom})." >&2
    echo "Adjust REF_* or GRADIENT_ACCUMULATION_STEPS, or set BATCH_SIZE_PER_GPU explicitly." >&2
    exit 1
  fi
  BATCH_SIZE_PER_GPU=$((TARGET_GLOBAL_BATCH_SIZE / denom))
fi
...
if [[ -z "${EXP_NAME:-}" ]]; then
  ...
  if [[ "${NODE_RANK}" == "0" ]]; then
    ...
    if [[ "${RESUME:-0}" == "1" && -s "${EXP_NAME_FILE}" ]]; then
      ...
      EXP_NAME="${CONFIG_NAME}_accel_ds_z${DEEPSPEED_STAGE}_${NUM_NODES}n${GPUS_PER_NODE}g_${TIMESTAMP}"
