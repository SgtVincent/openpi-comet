...
# ============================================================
# π0.5-KI joint query Single-Task Overfit FP16 Training (Formal Trial)
# Multi-Node: 4 nodes x 8 V100 = 32 GPUs
# Task: turning_on_radio (180 train / 20 val)
# Config: pi05_ki_joint_query_b1k-single_task-radio-ki_on_fp16
# Global batch size: 32 (1 per GPU x 32 GPUs)
# Training: 1 epoch
# Validation: every 100 steps
# Save checkpoint: every 200 steps
#
# CRITICAL: ZeRO-2 with NO CPU optimizer offload
#   (CPU offload causes step-0 parameter NaN on backbone;
#    GPU-resident Adam confirmed stable via 8-GPU smoke gate)
# FP16 stability profile: grad_clip=0.5, initial_scale_power=10,
...
# FP16 stability profile (must be set before accelerate launch)
export OPENPI_FP16_STABILITY_PROFILE=1
...
echo "============================================================"
echo "π0.5-KI joint query Single-Task Overfit FP16 Formal Trial"
echo "============================================================"
...
echo "  FP16_STABILITY_PROFILE: enabled"
echo "  CPU_OPTIMIZER_OFFLOAD: disabled (GPU-resident Adam)"
echo "============================================================"

# ============================================================
# Training hyperparameters
# ============================================================
CONFIG_NAME="${CONFIG_NAME:-pi05_ki_joint_query_b1k-single_task-radio-ki_on_fp16}"
...
if [[ -z "${EXP_NAME:-}" ]]; then
  ...
  EXP_NAME_FILE="${EXP_NAME_SYNC_DIR}/pi05_ki_joint_query_radio_fp16_${RUN_KEY}.txt"
  ...
  if [[ "${NODE_RANK}" == "0" ]]; then
    if [[ "${RESUME:-0}" == "1" && -s "${EXP_NAME_FILE}" ]]; then
      ...
      rm -f "${EXP_NAME_FILE}"
      EXP_NAME="pi05_ki_joint_query_single_task_radio_fp16_${NUM_NODES}n${GPUS_PER_NODE}g_${TIMESTAMP}"
...
# CRITICAL: Use ZeRO-2 with NO CPU offload (GPU-resident optimizer)
# CPU offload causes step-0 backbone parameter NaN due to CPUAdam fp16 corruption
ACCEL_CONFIG="${ACCEL_CONFIG:-configs/accelerate_ds_zero2_v100_fp16_no_offload.yaml}"
...
echo "Starting π0.5-KI joint query Single-Task Overfit FP16 Training"
...
echo "CPU optimizer offload: disabled (GPU-resident Adam)"
echo "FP16 stability profile: enabled"
