#!/usr/bin/env bash
# ============================================================================
# Formal pi0.5-KI **Variant B** (query-MSE) -- 4-node x 8 A100 BF16.
#
# Contract:
#   * B8/GPU x world 32 x GA1 = global batch 256
#   * stride=4, 4 epochs, anchor offsets 0/1/2/3
#   * BF16, DeepSpeed ZeRO-2, CPU optimizer offload
#   * No FAST action-token head (query-MSE arm)
#   * W&B project pi05_ki_a100, fresh run, NO resume
#
# Variant A lives in run_pi05_ki_formal_A_fast_bf16_4x8_a100.sh with a disjoint
# output tree and disjoint W&B/exp_name identity.
#
# CPU/read-only preflight:
#   OPENPI_LAUNCH_PREFLIGHT_ONLY=1 \
#   OPENPI_EXPECTED_CODE_COMMIT=$(git rev-parse HEAD) \
#   bash scripts/run_pi05_ki_formal_B_query_bf16_4x8_a100.sh
#
# FORMAL MERLIN ENTRYPOINT (the keepalive wrapper must be outermost):
#   OPENPI_KI_TRAINING_INNER=1 \
#   LAUNCHER=<repo>/scripts/run_pi05_ki_formal_B_query_bf16_4x8_a100.sh \
...
info() { printf '[pi05-ki-B-a100][%s] %s\n' "$(_ts)" "$*"; }
die() {
  printf '[pi05-ki-B-a100][%s] FATAL: %s\n' "$(_ts)" "$1" >&2
  shift || true
  for line in "$@"; do
    printf '[pi05-ki-B-a100]                         %s\n' "${line}" >&2
  done
  exit 2
}

# ---- Arm identity (hard-wired; this script is B-only) -----------------------
ARM="B"
ARM_LABEL="variantB_query_mse"
EXPECTED_CONFIG="pi05_ki_joint_query_b1k-full_task-ki_on_a100_bf16"
...
(( GLOBAL_BATCH_SIZE == 256 )) || die "global batch must be 256 (B8xW32xGA1), got ${GLOBAL_BATCH_SIZE}"
...
print("A100_BF16_ZERO2_PREFLIGHT_OK stage=2 offload=cpu bf16=true")
PY
...
# ---- B does not need FAST tokenizer ----------------------------------------
...
EXP_NAME="${EXP_NAME:-B_query_mse_a100_bf16_4n8g_${JOB_ID_SAFE}}"
...
[[ "${BASE_PI05_CKPT}" == "${FORMAL_BASE_PI05_CKPT}" ]] || die "base checkpoint pinned"
