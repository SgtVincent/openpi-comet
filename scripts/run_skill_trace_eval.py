...
def pick_representative_skills(
    registry: Dict[str, Dict[str, Any]],
    demo_data_path: Path,
    target_skills: List[str]
) -> List[Dict[str, Any]]:
    targets = set(target_skills)
...
def write_reports(out_dir: Path, rows: List[Dict[str, Any]]) -> None:
    ...
    with md_path.open("w") as f:
        f.write("# Skill Trace Evaluation Matrix (with dump_trace)\n\n")
...
def main() -> int:
    ...
    samples = pick_representative_skills(registry, args.demo_data_path, target_skills)
    ...
    for task_offset, task_name in enumerate(sorted(grouped)):
        ...
        try:
            ...
            for sample in grouped[task_name]:
                ...
                results.append(row)
                print(f"Result: runtime_ok={row['runtime_ok']}, success={row.get('success')}, result_type={row.get('result_type')}")
        finally:
            print(f"\nStopping server for {task_name}")
