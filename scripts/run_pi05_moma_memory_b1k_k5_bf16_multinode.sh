...
if [[ "${CONFIG_NAME}" != "pi05_moma_memory_b1k-k5" && "${CONFIG_NAME}" != "pi05_moma_memory_b1k-k5_smoke" ]]; then
  echo "ERROR: MoMA launcher refuses non-memory config: ${CONFIG_NAME}" >&2
  exit 2
fi
BASE_PI05_CKPT="${BASE_PI05_CKPT:-/mnt/bn/behavior-data-hl/chenjunting/repo/openpi-comet/checkpoints/pi05_base_pytorch}"
B1K_DATASET_ROOT="${B1K_DATASET_ROOT:-/mnt/bn/behavior-data-hl/chenjunting/data/2025-challenge-demos/}"
...
PERSISTENT_OUTPUT_ROOT="${PERSISTENT_OUTPUT_ROOT:-/mnt/bn/behavior-data-hl/chenjunting/repo/outputs/pi05_moma_memory_b1k_k5}"
...
if [[ -z "${EXP_NAME:-}" ]]; then
  ...
  RUN_KEY="${RUN_KEY//:/_}"
  RUN_KEY="${RUN_KEY// /_}"
  EXP_NAME_FILE="${EXP_NAME_SYNC_DIR}/moma_memory_b1k_k5_${RUN_KEY}.txt"
  ...
  if [[ "${NODE_RANK}" == "0" ]]; then
    if [[ "${RESUME:-0}" == "1" && -s "${EXP_NAME_FILE}" ]]; then
      ...
      EXP_NAME="pi05_moma_memory_b1k_k5_${NUM_NODES}n${GPUS_PER_NODE}g_${TIMESTAMP}"
...
echo "============================================================"
echo "MoMA-VLA HeldMemory-K BF16 launch"
