...
CONFIG_NAME="${CONFIG_NAME:-pi05_subtask_b1k-make_pizza_ann-skill_lr1e-4_5ep_sft}"
...
case "${SAVE_AT_EPOCH_END_ONLY}" in
  1|true|TRUE|True|yes|YES|y|Y)
    EXTRA_ARGS+=(--save-at-epoch-end-only)
    ;;
esac

echo "Starting PI0.5 subtask make_pizza SFT (annotations_skill supervision)"
