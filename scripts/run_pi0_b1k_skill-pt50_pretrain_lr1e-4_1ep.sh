...
CONFIG_NAME="${CONFIG_NAME:-pi0_b1k_skill-pt50_pretrain_lr1e-4_1ep}"
...
if [[ -z "${EXP_NAME:-}" ]]; then
  ...
  EXP_NAME_FILE="${EXP_NAME_SYNC_DIR}/pi0_pretrain_${RUN_KEY}.txt"
  if [[ "${NODE_RANK}" == "0" ]]; then
    mkdir -p "${EXP_NAME_SYNC_DIR}"
    if [[ "${RESUME:-0}" == "1" && -s "${EXP_NAME_FILE}" ]]; then
      EXP_NAME="$(cat "${EXP_NAME_FILE}")"
    else
      EXP_NAME="pi0_b1k_skill-pt50_pretrain_lr1e-4_1ep_${TIMESTAMP}"
...
echo "Starting pi0_b1k_skill-pt50_pretrain_lr1e-4_1ep training"
