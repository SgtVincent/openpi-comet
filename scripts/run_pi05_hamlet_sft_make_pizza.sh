...
# Full SFT training launcher for HAMLET baseline on make_pizza.
# Defaults are chosen to be cluster-friendly and easily overridable via env vars.
#
# Example:
#   CUDA_VISIBLE_DEVICES=0,1,2,3 NUM_TRAIN_EPOCHS=5 bash scripts/run_pi05_hamlet_sft_make_pizza.sh
#
# Smoke-style (low memory):
#   CUDA_VISIBLE_DEVICES=2 OPENPI_TRAIN_LORA_ONLY=1 BATCH_SIZE_PER_GPU=1 NUM_WORKERS=0 NUM_TRAIN_EPOCHS=1 \
...
CONFIG_NAME="${CONFIG_NAME:-pi05_hamlet_b1k-make_pizza_lr1e-4_5ep_sft}"
...
if [[ -n "${ARNOLD_WORKER_0_HOST:-}" && -n "${ARNOLD_WORKER_GPU:-}" && -n "${ARNOLD_WORKER_NUM:-}" && -n "${ARNOLD_ID:-}" ]]; then
  ...
  MASTER_PORT="${USER_MASTER_PORT:-29521}"
...
if [[ -z "${EXP_NAME:-}" ]]; then
  if [[ "${IS_MULTINODE}" == "1" ]]; then
    ...
    RUN_KEY="${RUN_KEY//:/_}"
    RUN_KEY="${RUN_KEY// /_}"
    EXP_NAME_FILE="${EXP_NAME_SYNC_DIR}/hamlet_make_pizza_sft_${RUN_KEY}.txt"
    if [[ "${NODE_RANK}" == "0" ]]; then
      mkdir -p "${EXP_NAME_SYNC_DIR}"
      if [[ "${RESUME:-0}" == "1" && -s "${EXP_NAME_FILE}" ]]; then
        EXP_NAME="$(cat "${EXP_NAME_FILE}")"
      else
        EXP_NAME="${CONFIG_NAME}_baseckpt_${NUM_TRAIN_EPOCHS}ep_${TIMESTAMP}"
...
echo "Starting HAMLET make_pizza SFT"
