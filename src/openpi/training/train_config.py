...
_CONFIGS = [
    ...
    *_PI05_KI_JOINT_QUERY_CONFIGS,
    # Without this line the MoMA memory configs are DEFINED but unselectable:
    # `cli()` builds its choices from this list, so the names are simply not
    # valid options, and `get_config()` falls back to pi05_b1k-base with only a
    # warning -- making "not registered" indistinguishable from "name typo".
    *_memory_configs(),
]
