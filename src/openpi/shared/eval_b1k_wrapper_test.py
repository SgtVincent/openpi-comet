...
def test_b1k_wrapper_rotate_session_clears_old_and_resets_new_session() -> None:
    model = _DummyModel()
    wrapper = B1KPolicyWrapper(_DummyPolicy(model), task_name="turning_on_radio", fine_grained_level=0)
    old_session_id = wrapper._session_id  # noqa: SLF001

    wrapper.rotate_session()

    assert wrapper._session_id != old_session_id  # noqa: SLF001
    assert model.cleared_sessions[-1] == old_session_id
    assert model.active_sessions[-1] == wrapper._session_id  # noqa: SLF001
    assert model.reset_sessions[-1] == wrapper._session_id  # noqa: SLF001
