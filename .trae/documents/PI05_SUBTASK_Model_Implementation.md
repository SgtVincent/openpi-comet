# PI05_SUBTASK Model Implementation（最终改动 / Concern / 使用方式）

本文档基于当前仓库代码、`PI05_FIX.md`、以及已有训练日志，整理 PI05_SUBTASK（π0.5 Subtask）特性的最终实现状态、关键改动、验证结果与仍需注意的限制。

## 目标与实现概览

PI05_SUBTASK 的目标是在同一个 VLM backbone 前向中，同时学习：
...
- 训练侧 PyTorch 模型：[pi05_subtask.py](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/models_pytorch/pi05_subtask.py)
- Action expert（同时产出 v_t 与 text logits）：[subtask_expert.py](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/models_pytorch/action_experts/subtask_expert.py)
- 配置入口：[pi05_subtask_config.py](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/models/pi05_subtask_config.py)

## 代码改动清单

### 新增核心文件

- [pi05_subtask_config.py](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/models/pi05_subtask_config.py)
  - `Pi05SubtaskConfig`
  - 暴露 `alpha`、`subtask_max_len`、`vocab_size` 等配置
  - 明确 JAX `create()` 未实现，仅支持 PyTorch 训练/推理路径
- [pi05_subtask.py](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/models_pytorch/pi05_subtask.py)
  - `PI05SubtaskPytorch`
  - `forward()` 返回 `{loss, flow_loss, ce_loss}`
  - 提供 `predict_subtask_tokens()`、`predict_subtask()`、`sample_actions_hierarchical()`
- [subtask_expert.py](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/models_pytorch/action_experts/subtask_expert.py)
  - `SubtaskActionExpert`
  - `compute_subtask_loss_train()` 单次 forward 同时计算文本 CE 与动作 velocity

### 关键修改文件

- [model.py](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/models/model.py)
  - `ModelType` 新增 `PI05_SUBTASK`
  - `Observation` 新增 `subtask_tokens/subtask_mask/subtask_loss_mask/subtask_ar_mask`
  - `preprocess_observation()` 与 `load_pytorch()` 路径支持 subtask
...
- [preprocessing_pytorch.py](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/models_pytorch/preprocessing_pytorch.py)
  - PyTorch preprocess 路径透传全部 subtask 字段，避免训练/推理中静默丢字段
- [transforms.py](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/transforms.py)
  ...
  - 新增 `TokenizeSubtaskInputs`，把 `prompt/state/subtask_text` 转成 subtask observation 字段
- [data_config.py](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/training/data_config.py)
  - `ModelTransformFactory` 新增 `PI05_SUBTASK` 分支
...
- [pretrain_config.py](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/training/pretrain_config.py)
  - 新增预训练配置 `pi05_subtask_b1k-pt50_cs32_bs64_lr2.5e-5_5ep`
- [sft_make_pizza_config.py](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/training/sft_make_pizza_config.py)
  - 新增单任务配置 `pi05_subtask_b1k-make_pizza_lr2.5e-6_5ep_sft`
...
- [train_pytorch.py](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/scripts/train_pytorch.py)
  ...
  - wandb 记录 `subtask/flow_loss` 与 `subtask/ce_loss`
...
- [policy.py](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/policies/policy.py)
  - `Policy.infer()` 现在支持两阶段 subtask 推理
...
- [run_pi05_subtask_train.sh](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/scripts/run_pi05_subtask_train.sh)
  - 通用 subtask 训练启动脚本
- [run_pi05_subtask_make_pizza_1ep.sh](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/scripts/run_pi05_subtask_make_pizza_1ep.sh)
  - 单任务 `make_pizza` 1000 step 验证脚本
  - 默认保守参数：`batch_size_per_gpu=1`、`num_workers=0`
- [run_pi05_subtask_validation.sh](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/scripts/run_pi05_subtask_validation.sh)
  - 运行 subtask 功能验证入口
- [test_pi05_subtask_feature.py](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/scripts/test_pi05_subtask_feature.py)
...
- `item["subtask_text"]`：固定取 `level=1` 的 `task` 字段，用于 subtask 的 CE 监督
...
- [TokenizeSubtaskInputs](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/transforms.py#L398-L445) 会生成全 0 的 subtask tokens/masks
- [PI05SubtaskPytorch.forward](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/models_pytorch/pi05_subtask.py#L95-L167) 会自动退化为仅 flow matching（`ce_loss=0`）
...
- subtask prefix/subtask mask 拼接现在有统一 helper，不再依赖脆弱的局部拼接逻辑
- preprocess 路径已透传全部 subtask 字段，避免 subtask 字段在中间层静默丢失
...
- `BaseModelConfig.load_pytorch()` 已支持加载 subtask checkpoint
...
预置 subtask pretrain config：

- `pi05_subtask_b1k-pt50_cs32_bs64_lr2.5e-5_5ep`

启动方式：

```bash
bash scripts/run_pi05_subtask_train.sh
```
...
torchrun --standalone --nnodes=1 --nproc_per_node=8 \
  scripts/train_pytorch.py pi05_subtask_b1k-pt50_cs32_bs64_lr2.5e-5_5ep \
  --exp_name pi05_subtask_train
```
...
- `pi05_subtask_b1k-make_pizza_lr2.5e-6_5ep_sft`

验证脚本：

```bash
bash scripts/run_pi05_subtask_make_pizza_1ep.sh
```

该脚本默认：

- `CONFIG_NAME=pi05_subtask_b1k-make_pizza_lr2.5e-6_5ep_sft`
...
`PI05SubtaskPytorch.sample_actions()` 现在有两种路径：
...
- [sample_actions](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/models_pytorch/pi05_subtask.py#L205-L223)
- [sample_actions_hierarchical](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/models_pytorch/pi05_subtask.py#L225-L244)

### 预测 subtask 文本

`predict_subtask_tokens()` / `predict_subtask()` 提供从 prefix 自回归生成 subtask 的能力：

- [predict_subtask_tokens](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/models_pytorch/pi05_subtask.py#L246-L321)
- [predict_subtask](file:///mnt/bn/robot-mllm-data-lf-3/mlx/users/chenjunting/repo/openpi-comet/src/openpi/models_pytorch/pi05_subtask.py#L323-L332)
...
- `checkpoints/console_logs/pi05_subtask_validation_light13.trace`
- `checkpoints/console_logs/pi05_subtask_validation_light13.log`

### 2) one-step subtask train smoke

真实环境 `openpi-comet-nas` 中，subtask forward/backward/optimizer step 已跑通。

产物：

- `checkpoints/console_logs/pi05_subtask_train_smoke_validation4.trace`
...
- `checkpoints/console_logs/pi05_subtask_real_train_validation4.log`
- `checkpoints/console_logs/pi05_subtask_real_train_validation4.status`
- `outputs/checkpoints/pi05_subtask_real_train_validation4/1/model.safetensors`
- `outputs/checkpoints/pi05_subtask_real_train_validation4/1/optimizer.pt`

### 4) 单任务 make_pizza 1000 step 验证

单任务实验 `pi05_subtask_make_pizza_step1000_validation_nw0` 已从日志确认完成 1000 step。

主日志：

- `outputs/logs/pi05_subtask_make_pizza_step1000_validation_nw0/rank0.log`
...
- 单任务 subtask 训练已经不是只停留在 one-step smoke
...
`Pi05SubtaskConfig.create()` 仍明确抛出 `NotImplementedError`，当前 subtask 仅支持 PyTorch 路径。

### 2) tokenizer 依赖外部下载

`SubtaskTokenizer` 与 `predict_subtask()` 都依赖 sentencepiece tokenizer；在无外网 / 无 GCS 权限环境下，需要提前准备缓存文件或替换下载逻辑。

### 3) checkpoint 仍使用宽松加载策略初始化 subtask

训练脚本在 `pytorch_model_name == "subtask"` 时使用 `strict=False` 加载已有权重，以兼容从 PI05 checkpoint 初始化 subtask 的场景。这意味着：
...
- subtask 训练主路径
- subtask-aware checkpoint 加载路径
- hierarchical inference 与 prompt cache
- 真实 checkpoint 的层级推理验证
- 真实数据的一步 formal 训练验证
- 单任务 `make_pizza` 的 1000 step 完整训练验证

换句话说，PI05_SUBTASK 在 openpi-comet 中已经从“搭骨架”推进到“可在真实数据上跑完整体验证”的状态，当前剩余问题主要集中在更大规模 runtime 稳定性与 subtask 语义选择，而不再是主功能缺失。
